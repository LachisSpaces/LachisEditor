...
namespace LachisEditor
{
   public class DBLoader : IProgressOperation
   {
      ...
      public static DataView GetDataView(string strTableName, string strFilterColumn, string strFilterType, string strFilterValue, bool blnUseTeamFilter)
      {
         return _dsCyanideDB.GetDataView(strTableName, strFilterColumn, strFilterType, strFilterValue, blnUseTeamFilter);
      }
            ...
            if (xnList != null)
               foreach (XmlNode x in xnList.ChildNodes)
               {
                  ...
                  else
                  {
                     ...
                     ForeignKeyList.Add(new ForeignKey(strFieldName, strItemsSource, strSelectedValuePath, strDisplayMemberPath, strFilter));
                  }
               }
         ...
         foreach (DBTable t in _dsCyanideDB.Tables)
         {
            if (t != null)
            {
               if (t.TableName != Const.CountryRegionTable)
               {
                  ...
                  xnTable.Attributes.Append(xdocSettings.CreateAttribute(Const.SettingsTableName)).Value = t.TableName;
                  xnTable.Attributes.Append(xdocSettings.CreateAttribute(Const.SettingsTablePrimaryKey)).Value = t.PrimaryKeyName;
                  if (t.ForeignKeyList.Count > 0)
                  {
                     ...
                     foreach (ForeignKey fk in t.ForeignKeyList)
                     {
                        ...
                        xnFK.Attributes.Append(xdocSettings.CreateAttribute(Const.SettingsTableFKFieldName)).Value = fk.FieldName;
                        xnFK.Attributes.Append(xdocSettings.CreateAttribute(Const.SettingsTableFKItemsSource)).Value = fk.ItemsSource;
                        xnFK.Attributes.Append(xdocSettings.CreateAttribute(Const.SettingsTableFKSelectedValuePath)).Value = fk.SelectedValuePath;
                        xnFK.Attributes.Append(xdocSettings.CreateAttribute(Const.SettingsTableFKDisplayMemberPath)).Value = fk.DisplayMemberPath;
                        xnFK.Attributes.Append(xdocSettings.CreateAttribute(Const.SettingsTableFKFilter)).Value = fk.Filter;
                  ...
                  if (t.LargeSingleList.Count > 0)
                  {
                     ...
                     foreach (string s in t.LargeSingleList)
                        xnList.AppendChild(xdocSettings.CreateElement(Const.SettingsTableLargeSingle)).InnerText = s;
                     xnTable.AppendChild(xnList);
                  }
   ...
   internal class LatestUsageComparer : IComparer
   {
      public int Compare(object x, object y)
      {
         FileInfo objX = null;
         FileInfo objY = null;
         try { objX = ((DirectoryInfo)x).GetFiles(Const.SettingsFileName)[0]; }
         catch { return 1; }
         try { objY = ((DirectoryInfo)y).GetFiles(Const.SettingsFileName)[0]; }
         catch { return -1; }
